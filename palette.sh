...
has() {
    ...
    # busybox-w32's shell doesn't return a full path for builtin applets,
    # causing a false negative. This is a workaround for that.
...
wmic_read() {
    # This function is used to remove useless garbage from wmic output.
    # A command like 'wmic os get Caption' will output:
    # Caption
    # Microsoft Windows 7 Enterprise
    # <BLANK LINE>
    # This function transforms the output into:
    # Microsoft Windows 7 Enterprise
    {
        read -r _
        read -r line
...
get_title() {
    ...
    # to the 'hostname' command, then to the '/etc/hostname' file
    if [ -z "$hostname" ]; then
        if [ -n "$HOSTNAME" ]; then
            hostname="$HOSTNAME"
...
get_pkgs() {
    ...
    case $os in
        (Linux*)
            ...
            has bonsai     && count_single_pkg bonsai 0 bonsai list
            has crux       && count_single_pkg pkginfo 0 pkginfo -i
            has pacman-key && count_single_pkg pacman 0 pacman -Qq
            has dpkg       && count_single_pkg dpkg 0 dpkg-query -f '.\n' -W
            has rpm        && count_single_pkg rpm 0 rpm -qa
            has xbps-query && count_single_pkg xbps 0 xbps-query -l
            has apk        && count_single_pkg apk 0 apk info
            has guix       && count_single_pkg guix 0 guix package --list-installed
            has opkg       && count_single_pkg opkg 0 opkg list-installed
            has radula     && count_single_pkg radula 0 radula -ch
            has flatpak    && count_single_pkg flatpak 0 flatpak list
            has snap       && count_single_pkg snap 0 snap list
            has urpmq      && count_single_pkg urpmq 0 urpmq --list-media active
            has birb       && count_single_pkg birb 0 birb --list-installed

            # Directories containing packages.
            has kiss       && count_single_pkg kiss 0 printf '%s\n' /var/db/kiss/installed/*/
            has cpt-list   && count_single_pkg cpt-list 0 printf '%s\n' /var/db/cpt/installed/*/
            has brew       && count_single_pkg homebrew 0 printf '%s\n' "$(brew --cellar)/"* "$(brew --caskroom)/"*
            has emerge     && count_single_pkg portage 0 printf '%s\n' /var/db/pkg/*/*/
            has pkgtool    && count_single_pkg pkgtool 0 printf '%s\n' /var/log/packages/*
            has eopkg      && count_single_pkg eopkg 0 printf '%s\n' /var/lib/eopkg/package/*

            # 'nix' requires two commands.
            has nix-store  && count_single_pkg nix sh -c 'nix-store -q --requisites /run/current-system/sw; nix-store -q --requisites ~/.nix-profile'
        ;;
        ...
        (CYGWIN*|MSYS*|MINGW*|Windows_NT)
            ...
            has scoop && {
                ...
                count_single_pkg scoop 1 printf '%s\n' "$scoopapps"/*
            }
            has winget     && count_single_pkg winget 0 winget list
            has chocolatey && count_single_pkg chocolatey 0 choco list --local-only
            has pacman-key && count_single_pkg pacman 0 pacman -Qq
        ;;

        (Darwin*)
            # Commands which print packages one per line.
            has pkgin      && count_single_pkg pkgin 0 pkgin list
            has dpkg       && count_single_pkg dpkg 0 dpkg-query -f '.\n' -W
            ...
            has port       && {
                ...
                case "$pkg_list" in
                    ...
                    (*)
                        count_single_pkg macports 0 printf '%s' "$pkg_list"
                    ;;
                esac
            }
        ;;
        ...
        (FreeBSD*|DragonFly*)
            count_single_pkg FreeBSD 0 pkg info
        ;;

        (OpenBSD*)
            count_single_pkg OpenBSD 0 printf '%s\n' /var/db/pkg/*/
        ;;

        (NetBSD*)
            count_single_pkg pkgsrc 0 pkg_info
        ;;

        (Haiku)
            count_single_pkg Haiku 0 printf '%s\n' /boot/system/package-links/*
        ;;

        (Minix)
            count_single_pkg Minix 0 printf '%s\n' /usr/pkg/var/db/pkg/*/
        ;;

        (SunOS)
            has pkginfo && count_single_pkg pkginfo 0 pkginfo -i
            has pkg     && count_single_pkg pkg 0 pkg list
        ;;

        (IRIX)
            count_single_pkg IRIX 3 versions -b
        ;;

        (SerenityOS)
            count_single_pkg SerenityOS 0 sh -c "while IFS=" " read -r type _; do [ \"\$type\" != dependency ] && printf '\n'; done < /usr/Ports/packages.db"
        ;;
    esac

    # package managers that are on many different systems
    has cargo      && count_single_pkg cargo 0 sh -c 'cargo install --list | grep -v "^ "'
    has pipx       && count_single_pkg pipx 0 pipx list
...
get_disk() {
    ...
    df "$@" "$PF_DISKPATH" | while read -r name full used _ perc _; do
...
get_resolution() {
    case $os in
        ...
        ;;
        Darwin*)
            resolution=$(system_profiler SPDisplaysDataType | grep Resolution | awk '{print $2 "x" $4}')
        ;;
    esac

    log res "$resolution" >&6
}
